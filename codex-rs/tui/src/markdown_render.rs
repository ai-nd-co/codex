    ...
    I: Iterator<Item = Event<'a>>,
{
    ...
    fn handle_table_event(&mut self, event: &Event<'a>) -> bool {
        ...
        match event {
            ...
            Event::Text(text) => table.push_text(text.as_ref()),
            Event::Code(code) => table.push_text(code.as_ref()),
            Event::SoftBreak | Event::HardBreak => table.push_space(),
            _ => {}
        }
        true
    }
    ...
    fn render_table(&mut self, table: TableState) {
        ...
        for line in table_output.to_string().lines() {
