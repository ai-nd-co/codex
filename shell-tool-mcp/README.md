...
Because `@ai-nd-co/codex-shell-tool-mcp` intercepts `execve(2)` calls directly, it _always_ knows the full path to the program being executed. In turn, this makes it possible to provide stronger guarantees on how [Codex `.rules`](https://developers.openai.com/codex/local-config#rules-preview) are enforced.
...
```bash
npx -y @ai-nd-co/codex-shell-tool-mcp --version
```
...
codex --disable shell_tool \
  --config 'mcp_servers.bash={command = "npx", args = ["-y", "@ai-nd-co/codex-shell-tool-mcp"]}'
```
...
command = "npx"
args = ["-y", "@ai-nd-co/codex-shell-tool-mcp"]
```

Note when the `@ai-nd-co/codex-shell-tool-mcp` launcher runs, it selects the appropriate native binary to run based on the host OS/architecture. For the Bash wrapper, it inspects `/etc/os-release` on Linux or the Darwin major version on macOS to try to find the best match it has available. See [`bashSelection.ts`](https://github.com/ai-nd-co/codex/blob/main/shell-tool-mcp/src/bashSelection.ts) for details.
...
This package wraps the `codex-exec-mcp-server` binary and its helpers so that the shell MCP can be invoked via `npx -y @ai-nd-co/codex-shell-tool-mcp`. It bundles:
