...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="GitHub repo slug that owns the workflow run (default: openai/codex).",
    )
    parser.add_argument(
        "--target",
        dest="targets",
        action="append",
        help=(
            "Limit installation to specific target triples. May be repeated. "
            "Defaults to all supported targets for the selected components."
        ),
    )
    parser.add_argument(
        "--component",
...
def main() -> int:
    ...
    selected_targets = list(args.targets or [])
    ...
    if "rg" in components:
        with _gha_group("Fetch ripgrep binaries"):
            ...
            fetch_rg(
                vendor_dir,
                selected_targets or DEFAULT_RG_TARGETS,
